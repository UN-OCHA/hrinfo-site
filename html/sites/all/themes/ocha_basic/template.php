 ...
 *
 * Overrides theme_field_multiple_value_form() to drag-n-drop reordering where
 * it's not needed.
 *
 * @param $variables
 ...
 * @ingroup themeable
 */
function ocha_basic_field_multiple_value_form($variables) {
  // Check for field.
  $fields = array(
    'field_phones',
    'field_organizations',
  ...
  $entity = $variables['element'][0];
  if (!((isset($entity['#bundle']) && in_array($entity['#bundle'], $bundles)) ||
    (isset($entity['target_id']['#bundle']) && in_array($entity['target_id']['#bundle'], $bundles)))) {
    return theme_field_multiple_value_form($variables);
  }
  ...
  if ($element['#cardinality'] > 1 || $element['#cardinality'] == FIELD_CARDINALITY_UNLIMITED) {
    ...
    foreach ($items as $key => $item) {
      ...
      if (!empty($item['#delta']) && empty($item['#default_value'])) {
        $row['class'] = array('hr-add-more');
      }
      $rows[] = $row;
    }
